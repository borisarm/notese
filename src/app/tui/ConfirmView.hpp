...
public:
    static ftxui::Element render_note_delete(const TuiState& state) {
        ...
        auto idx = std::clamp(state.selection.note_selected, 0, (int)state.notes.size() - 1);
    ...
    static ftxui::Element render_reminder_delete(const TuiState& state) {
        ...
        auto idx = std::clamp(state.selection.reminder_selected, 0, (int)state.reminders.size() - 1);
